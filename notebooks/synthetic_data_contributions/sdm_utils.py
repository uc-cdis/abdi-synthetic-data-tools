...
def read_schemas(schemas,exclude_nodes=None,exclude_props=None):
    ...
    if exclude_nodes == None:
        exclude_nodes = ['program','core_metadata_collection','root','metaschema','data_release','_settings','_terms','_definitions']
    ...
    for schema in schemas:
        ...
        prop_counts.append(average_prop_count) # average number of properties per node per schema
        schema_name = schema.split('/')[-1].split('_')[0]
        print(f"\t{schema_name}: {len(node_defs)} nodes with ~{int(average_prop_count)} properties per node.")
        dms[schema_name] = dm
    print(f"Average node count per schema: {sum(node_counts)/len(node_counts)}, average number of properties per node: {int(sum(prop_counts)/len(prop_counts))}.")
    return dms
...
def create_master_project(dms):
    ...
    project_props = []
    prop_names = sorted(list(set([p['name'] for p in [item for sublist in [pdef['properties'] for pdef in pdefs] for item in sublist]])))
    for prop in prop_names: # prop = prop_names[0]
        prop_nodes = [pdef for pdef in pdefs if prop in [p['name'] for p in pdef['properties']]]
        ...
        project_props.append(prop_def) # add the property definition to the master project def
    # get to required properties from each project node
    all_required = sorted(list(set([item for sublist in [p['required'] for p in pdefs] for item in sublist if item in prop_names])))
        ...
        for node in nodes: # node = nodes[1]
            ...
            if node not in suborder:
                ...
                if False in [i in suborder for i in targets]: 
                    ...
                    parents_order = [item for item in suborder if item in targets] # get existing order for each parent in list
                    suborder[node] = max([suborder[parent] for parent in parents_order]) + 1 # add the node with the max order of the parents + 1
...
def get_node_parents(node,dm):
    ...
    ordered_parents = [n for n in sorted(list(set(parents)), key=lambda x: get_submission_order(dm)[x])]
...
def get_sdm_synonyms(sdm,synonymous_nodes=None):
    ...
    if synonymous_nodes is None:
        synonymous_nodes = [['study','dataset','clinical_trial','collection','research'],
    ...
    if len(syn_lists) > 0:
        for syn_list in syn_lists: # syn_list = syn_lists[0]
            ...
            for syn in syns: # syn = syns[0] # case
                ...
                syn_props = [p for p in syn_node['properties'] if p['name'] != f"{syn}.id"]
                ...
                for link in syn_links: # link=syn_links[0]
                    ...
                    if link_prop in [p['name'] for p in syn_props]:
                        ...
                        if 'description' in p:
                            p['description'] = p['description'].replace(syn,chosen_name)
                chosen_node['properties'] += [p for p in syn_props if p['name'] not in [c['name'] for c in chosen_node['properties']]]
                chosen_node['links'] = list(set(chosen_node['links'] + syn_node['links']))
                # replace the syn with chosen_name in any 'links', 'required' and 'properties' for each other node
                for node_def in [n for n in udm['nodes'] if n['name'] != chosen_name and n['name'] != syn and n['name'] != 'project']:
                    ...
                    for i in range(0,len(node_def['properties'])):
                        if node_def['properties'][i]['name'] == f"{syn}.id":
                            ...
                            node_def['properties'][i]['description'] = node_def['properties'][i]['description'].replace(syn,chosen_name)
...
def fix_links(sdm):
    ...
    for node in sdm_nodes: # node = sdm_nodes[0]
        ...
        node_def['properties'] = [p for p in node_def['properties'] if p['name'].replace('.id','') not in remove_links]
        node_def['required'] = [r for r in node_def['required'] if r.replace('.id','') not in remove_links]
        if len(node_def['links']) == 1:
            node_def['required'] = list(set(node_def['required'] + [node_def['links'][0]+'.id']))
    return sdm
...
def select_optional_properties(sdm, min_props_perc, max_props_perc, keep_required=False):
    """
    ...
    for node_def in [n for n in udm['nodes']]:
        if 'required' in node_def and keep_required:
            ...
            keep_props = [p['name'] for p in node_def['properties'] if p['name'].endswith('.id') or p['name'] in required_props]
        else:
            keep_props = [p['name'] for p in node_def['properties'] if p['name'].endswith('.id')]
        optional_props = [p['name'] for p in node_def['properties'] if p['name'] not in keep_props]
        if len(optional_props) > 0: # get random integer of optional properties to keep between min and max percentages            
            ...
            keep_optional = random.sample(optional_props,num_props) # randomly select optional properties to keep
            all_keep = keep_optional+keep_props
        else:
            all_keep = keep_props
        updated_props = [] # update node_def['properties'] making sure property names in node_def['properties'] are all unique
        for prop in node_def['properties']:
            if prop['name'] in all_keep and prop['name'] not in [p['name'] for p in updated_props]:
...
def truncate_sdm_descriptions(sdm,desc_limit=1000,log_file="description_truncation_log.json"):
    """
    ...
    for node_def in sdm['nodes']:
        if 'description' in node_def:
            ...
            if len(node_desc) > desc_limit:
                ...
                print(f"\tTruncating description from {len(desc)} to {len(tdesc)} characters.")
