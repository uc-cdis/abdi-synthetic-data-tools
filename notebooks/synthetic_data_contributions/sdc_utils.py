        ...
        for node in nodes: # node = nodes[1]
            ...
            if node not in suborder:
                ...
                if False in [i in [i for i in suborder] for i in parents]: 
                    ...
                    parents_order = [item for item in suborder if item in parents] # get existing order for each parent in list
...
def get_file_nodes(dm,use_file_name=True):
    ...
    for node_def in dm['nodes']: # node_def = dm['nodes'][1]
        ...
        #     file_nodes.append(node)
        if use_file_name and 'properties' in node_def and any(p['name'] for p in node_def['properties'] if p['name'] == 'file_name'):
...
def get_node_parents(node,dm):
    ...
    ordered_parents = sorted(list(set(parents)), key=lambda x: get_submission_order(dm)[x])
...
def organize_headers(headers,node,dm):
    ...
    if 'required' in node_def:
        for prop in sorted(node_def['required']):
            if prop in headers and prop not in sorted_headers and prop not in foreign_keys:
                sorted_headers.append(prop)
    for prop in sorted([h for h in headers if h not in sorted_headers and h not in foreign_keys]): # add any remaining props
        sorted_headers.append(prop)
    for prop in [h for h in foreign_keys if h not in sorted_headers and h in headers]: # add foreign keys
        sorted_headers.append(prop)
...
def randomize_headers(node,dm,keep_min,keep_max,hard_limit,keep_required=False):
    ...
    if keep_required and 'required' in node_def:
        random_headers += [h for h in node_def['required'] if h in headers] # keep required properties as defined in node schema
    ...
    if len(random_headers) > hard_limit:
        ...
        random_headers = random.sample([h for h in random_headers if '.id' not in h],hard_limit-len([h for h in random_headers if h.endswith('.id')])) + [h for h in random_headers if h.endswith('.id')]
...
def validate_headers(gheaders,all_headers):
    ...
    for node in gheaders:
        ...
        for header in headers:
            if header not in [item for sublist in list(all_headers.values()) for item in sublist] and header != 'project_id' and header != f'{node}_id':
    ...
    for node in all_headers:
        ...
        for header in all_headers[node]:
            ...
            if header not in [item for sublist in list(gheaders.values()) for item in sublist]:
...
def write_node_group_schema(all_headers,dm,schema_name,groupname,groupdir,write_file=True):
    ...
    for node in all_headers: # node = list(all_headers)[0]
        ...
        node_def['properties'] = [p for p in node_def['properties'] if p['name'] in all_headers[node]]
        node_def['required'] = [p for p in node_def['required'] if p in all_headers[node]]
